...
class OverlayService : Service() {
    ...
    private suspend fun processCommand(text: String): Boolean {
        ...
        container.winGo.voice.tryHandle(text)?.let { winGoReply ->
            ...
            return speakWithBargeIn(winGoReply)
        }
        LocalIntentRouter.match(text)?.let { command ->
            ...
            if (command is JarvisCommand.PauseTask) { container.taskEngine.pause(); return speakWithBargeIn("Paused.") }
            if (command is JarvisCommand.ResumeTask) { container.taskEngine.resume(); return speakWithBargeIn("Resuming.") }
            if (command is JarvisCommand.RetryLastTask) {
                val target = lastUserRequestText
                return if (target.isNullOrBlank()) speakWithBargeIn("There's nothing to retry yet.")
                else processCommand(target)
            }
            // A locally-matched command can still be destructive (e.g. ForgetMemory) — same
            // confirm-before-acting rule as everywhere else, not just AI-issued commands.
            if (container.confirmationManager.required(command)) {
                pendingConfirmation = command
                return speakWithBargeIn("${command.describe()} Say yes or no.")
            }
            ...
            val reply = brainReply ?: when (val result = container.actionExecutor.execute(command)) {
        ...
        val knowledgeContext = container.knowledgeBase.compactPrompt(container.knowledgeBase.retrieve(text))
            ...
            onSuccess = { result ->
                ...
                interrupted = when {
                    ...
                    command != null && !container.confirmationManager.required(command) -> {
                        ...
                        val brainReply = container.brainCommandExecutor.execute(command)
                        val reply = brainReply ?: when (val execution = container.actionExecutor.execute(command)) {
    ...
    private suspend fun handlePendingConfirmation(pending: JarvisCommand, answerText: String): Boolean {
        ...
        return when (answer) {
            true -> {
                ...
                setState(VoiceState.THINKING)
                val brainReply = container.brainCommandExecutor.execute(pending)
                val reply = brainReply ?: when (val result = container.actionExecutor.execute(pending)) {
