...
@Composable
fun SettingsScreen(onOpenDashboard: () -> Unit = {}, onOpenPermissions: () -> Unit = {}, viewModel: SettingsViewModel = viewModel()) {
    ...
    Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
        TopAppBar(title = { Text("System Control") })
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text("Silent mode")
                Text(
                    "JARVIS keeps replying in text and still executes commands, but never " +
                        "speaks out loud until you turn this back off.",
                    color = JarvisTextSecondary,
                    style = MaterialTheme.typography.labelSmall
                )
            }
            Switch(checked = state.silentModeEnabled, onCheckedChange = viewModel::updateSilentMode)
        }
        Row(
            modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 4.dp),
            horizontalArrangement = Arrangement.SpaceBetween
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text("Focus mode")
                Text(
                    "Suppresses JARVIS's own notifications (task-completed, low-battery) " +
                        "while everything still runs normally underneath.",
                    color = JarvisTextSecondary,
                    style = MaterialTheme.typography.labelSmall
                )
            }
            Switch(checked = state.focusModeEnabled, onCheckedChange = viewModel::updateFocusMode)
        }
        Row(
            modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 4.dp),
            horizontalArrangement = Arrangement.SpaceBetween
        ...
        SectionLabel("System Intelligence")
        Text(
            "Run a full self-check across AI, memory, voice, accessibility, automation, the " +
                "database, HUD, live wallpaper, network and permissions — or review how JARVIS's " +
                "tasks have been performing.",
            color = JarvisTextSecondary,
            style = MaterialTheme.typography.bodyMedium,
            modifier = Modifier.padding(16.dp)
        )
        OutlinedButton(
            ...
            modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 4.dp)
        ) { Text("Open permission center") }
        OutlinedButton(
            onClick = { viewModel.runDiagnostic() },
            modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 4.dp)
        ) { Text("Run JARVIS diagnostic") }
        OutlinedButton(
    ...
    diagnosticReport?.let { report ->
        AlertDialog(
            ...
            text = { Text(report, fontFamily = androidx.compose.ui.text.font.FontFamily.Monospace, fontSize = 12.sp) },
            confirmButton = { TextButton(onClick = { viewModel.dismissDiagnostic() }) { Text("Close") } }
        )
    }
}
