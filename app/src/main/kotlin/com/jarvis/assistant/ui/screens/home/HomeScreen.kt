...
fun HomeScreen(
    ...
    // #25 NOTIFICATIONS — a real, debounced low-battery warning (fires once per drop below the
    // threshold, not once per telemetry tick).
    androidx.compose.runtime.LaunchedEffect(telemetry.batteryPercent, telemetry.charging) {
        com.jarvis.assistant.agent.JarvisNotifier.maybeNotifyLowBattery(
            viewModel.getApplication(),
            viewModel.securePrefsForNotifications(),
    ...
    Box(
        ...
        Column(
            ...
            Box(modifier = Modifier.size(320.dp), contentAlignment = Alignment.Center) {
                JarvisCore(
                    state = voiceState,
                    modifier = Modifier.size(260.dp).pointerInput(voice) {
                        detectTapGestures(
                            onTap = {
                                if (voice == VoiceState.LISTENING) viewModel.cancelListening()
                                else viewModel.startListening(null)
                            },
                            onLongPress = { radialMenuOpen = true }
                        )
                    }
                )
                // #20 UI ARCHITECTURE — a HUD-style radial quick-access menu, additive to (not
                // replacing) the bottom nav: long-press the core to reveal it.
                RadialQuickMenu(
                    visible = radialMenuOpen,
                    onDismiss = { radialMenuOpen = false },
                    items = listOf(
                        RadialMenuItem("DIAG") { viewModel.sendMessage("run jarvis diagnostic", true) },
                        RadialMenuItem("DASH", onOpenDashboard),
                        RadialMenuItem("PERM", onOpenPermissions),
                        RadialMenuItem("MEM", onOpenMemory),
                        RadialMenuItem("CTRL", onOpenSettings)
                    )
                )
            }
...
private fun TelemetryStrip(t: com.jarvis.assistant.hud.DeviceTelemetry) {
    ...
    Column {
        ...
        // #9 DEVICE UNDERSTANDING — CPU core count. No live-load ring here: modern Android has
        // no public API for real-time CPU load, so this stays an honest static fact rather
        // than a fake-looking gauge with nothing real driving it.
        t.cpuCoreCount?.let { cores ->
            Text(
                "CPU: $cores CORES",
...
data class RadialMenuItem(val label: String, val onClick: () -> Unit)

/** #20 UI ARCHITECTURE — HUD-style radial quick-access menu, additive to the bottom nav bar
 * (not a replacement — see JARVIS_ULTRA_PASS_NOTES.md for why a full nav-shell replacement
 * wasn't attempted). Opened by long-pressing the Command Center core. Deliberately built with
 * only APIs already proven elsewhere in this file (Canvas/Modifier/graphicsLayer) rather than
 * androidx.compose.animation's AnimatedVisibility/fadeIn/scaleIn — that's a separate Gradle
 * artifact not explicitly declared in this project's dependencies, and pulling it in only
 * transitively is a real compile-time risk I can't verify without an actual build. */
@Composable
private fun RadialQuickMenu(visible: Boolean, onDismiss: () -> Unit, items: List<RadialMenuItem>) {
    if (!visible) return
    Box(
        modifier = Modifier.size(320.dp).clickable { onDismiss() },
        ...
        items.forEachIndexed { index, item ->
            ...
            val x = (radius * kotlin.math.cos(angle)).dp
            val y = (radius * kotlin.math.sin(angle)).dp
            Surface(
                color = androidx.compose.ui.graphics.Color(0xEE06131C),
                shape = CircleShape,
                modifier = Modifier
                    .offset(x = x, y = y)
                    .size(52.dp)
                ...
                Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    Text(item.label, color = JarvisCyan, fontSize = 9.sp, fontFamily = FontFamily.Monospace)
                }
            }
        }
    }
