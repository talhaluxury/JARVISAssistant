...
/**
 * #50 JARVIS SYSTEM PROMPT ARCHITECTURE — assembles the final request from the independent
 * blocks in [PromptComponents] plus the fixed AVAILABLE_TOOLS action-list block below, instead
 * of one giant hardcoded prompt. The model is instructed to answer normally, and ONLY when the
 * user's request maps to a real, supported phone action, to append a single fenced
 * ```jarvis_command block containing JSON matching one of the allowed types. The CommandEngine
 * is the only thing that ever executes those actions, and it validates the type/fields before
 * doing anything — the model's JSON is a suggestion, never code, and never trusted blindly.
 */
object PromptBuilder {
    ...
    fun systemPrompt(
        ...
        previousActionSummary: String? = null,
        knowledgeContext: String = ""
    ): String = buildString {
        appendLine(PromptComponents.corePersonality())
        appendLine()
        appendLine(PromptComponents.languageContext(languageHint))
        appendLine()
        appendLine(PromptComponents.deviceContext(phoneContext, appRegistry))
        appendLine()
        if (capabilitySnapshot.isNotBlank()) {
            appendLine(PromptComponents.capabilityContext(capabilitySnapshot))
            appendLine()
        }
        appendLine(PromptComponents.taskContext(currentTaskDescription, previousActionSummary))
        appendLine()
        appendLine(PromptComponents.conversationalIntelligenceNote())
        appendLine()
        appendLine(PromptComponents.confidenceRule())
        appendLine()
        appendLine(PromptComponents.securityRules())
        appendLine()
        appendLine(PromptComponents.responseStyle())
        appendLine()
        appendLine(AVAILABLE_TOOLS)
        appendLine()
        if (knowledgeContext.isNotBlank()) {
            appendLine(PromptComponents.knowledgeContext(knowledgeContext))
            appendLine()
        }
        appendLine(PromptComponents.memoryContext(memoryContext))
    }

    /** #51 TOOL-AWARE AI — the closed list of action types the AI may propose. Never expose an
     * unavailable tool here: this list only ever grows alongside a real CommandEngine case. */
    private val AVAILABLE_TOOLS = """
        ...
        - {"type":"MEMORY_QUERY"} — use when the user asks "what do you remember about me" / "mujhe kya yaad hai".
        - {"type":"FORGET_MEMORY","query":"<text to match and remove, or omit to ask which>"} — "forget that", "delete my saved preferences".
        - {"type":"RUN_DIAGNOSTIC"} — use when the user asks JARVIS to check itself / run a diagnostic.
        - {"type":"LIST_INSTALLED_APPS"} — "which apps are installed?", "find the browser".
        - {"type":"CLOSE_APP"} — "close this app", "close the current app if possible". Android
          has no API for one app to force-close another, so this returns to the home screen
          instead and says so honestly — never claim the app was actually terminated.
        ...
        - {"type":"PAUSE_TASK"} — pause the in-progress multi-step task.
        - {"type":"RESUME_TASK"} — resume a paused task.
        - {"type":"RETRY_LAST_TASK"} — re-run the most recent task/command ("phir se karo").
        ...
        - {"type":"AUTOMATE","package":"<android package name if known, e.g. com.whatsapp, or omit
          ...
          of:
            ...
            {"action":"swipe","value":"up|down|left|right"}
