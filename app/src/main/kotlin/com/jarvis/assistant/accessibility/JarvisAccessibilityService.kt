...
class JarvisAccessibilityService : AccessibilityService() {
    ...
    private fun tryRunNextStep() {
        ...
        val handled = try {
            when (step) {
                ...
                is AutomationStep.Swipe -> performSwipe(step.direction)
            }
        } catch (e: Exception) {
            false
        }
    ...
    /** #11 ACCESSIBILITY AUTOMATION — a real touch-gesture swipe, for UI (carousels, image
     * viewers, some custom lists) that doesn't expose a scrollable accessibility node and so
     * never responds to [scroll]'s ACTION_SCROLL_FORWARD/BACKWARD. dispatchGesture only reports
     * whether the gesture was accepted for dispatch, which is the same strength of signal the
    ...
    private fun performSwipe(direction: SwipeDirection): Boolean {
        ...
        when (direction) {
            SwipeDirection.UP -> { path.moveTo(w / 2, h * 0.75f); path.lineTo(w / 2, h * 0.25f) }
            SwipeDirection.DOWN -> { path.moveTo(w / 2, h * 0.25f); path.lineTo(w / 2, h * 0.75f) }
            SwipeDirection.LEFT -> { path.moveTo(w * 0.8f, h / 2); path.lineTo(w * 0.2f, h / 2) }
            SwipeDirection.RIGHT -> { path.moveTo(w * 0.2f, h / 2); path.lineTo(w * 0.8f, h / 2) }
        }
        val gesture = android.accessibilityservice.GestureDescription.Builder()
            .addStroke(android.accessibilityservice.GestureDescription.StrokeDescription(path, 0, 220))
            .build()
