    ...
    fun parse(commandJson: String?): JarvisCommand? {
        ...
        return try {
            ...
            when (json.optString("type").uppercase()) {
                ...
                "RUN_DIAGNOSTIC", "DIAGNOSTIC" -> JarvisCommand.RunDiagnostic
                "MEMORY_QUERY" -> JarvisCommand.MemoryQuery
                "FORGET_MEMORY" -> JarvisCommand.ForgetMemory(json.optString("query", ""))
                "PAUSE_TASK", "PAUSE" -> JarvisCommand.PauseTask
                "RESUME_TASK", "RESUME" -> JarvisCommand.ResumeTask
                "RETRY_LAST_TASK", "RETRY" -> JarvisCommand.RetryLastTask
                "SHOW_LEARNING_STATS" -> JarvisCommand.ShowLearningStats
                "LIST_INSTALLED_APPS" -> JarvisCommand.ListInstalledApps
                "CLOSE_APP" -> JarvisCommand.CloseCurrentApp
    ...
    private fun parseAutomate(json: JSONObject): JarvisCommand.Automate? {
        ...
        for (i in 0 until stepsArray.length()) {
            ...
            val step = when (stepJson.optString("action").lowercase()) {
                ...
                "swipe" -> when (stepJson.optString("value").lowercase()) {
                    "up" -> AutomationStep.Swipe(SwipeDirection.UP)
                    "down" -> AutomationStep.Swipe(SwipeDirection.DOWN)
                    "left" -> AutomationStep.Swipe(SwipeDirection.LEFT)
                    "right" -> AutomationStep.Swipe(SwipeDirection.RIGHT)
                    else -> null
                }
                else -> null
            }
