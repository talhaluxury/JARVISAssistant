...
class AndroidActionExecutor(private val context: Context, private val securePrefs: SecurePrefs) {
    ...
    fun execute(command: JarvisCommand): ExecutionResult = try {
        if (command::class in screenAutomationCommands && !securePrefs.screenAutomationEnabled) {
            ...
            JarvisCommand.ListInstalledApps -> listInstalledApps()
            JarvisCommand.CloseCurrentApp -> closeCurrentApp()
            ...
            // HUD commands are always intercepted and handled by HudCommandExecutor, and brain
            // commands (diagnostic/memory/pause/resume/retry/stats) by BrainCommandExecutor,
            // before reaching this executor (see AssistantViewModel). These branches only exist
            // to satisfy the compiler's exhaustiveness check and are not expected to run.
    ...
    /** #10 APP INTELLIGENCE — "which apps are installed?" Reuses the same public-launcher
     * query AppRegistry uses (not a fresh dependency — just constructed locally here to avoid
     * a wider constructor change), so the answer always matches what OPEN_APP can actually
     * resolve by name. */
    private fun listInstalledApps(): ExecutionResult {
        val apps = com.jarvis.assistant.agent.AppRegistry(context).snapshot()
        if (apps.isEmpty()) return ExecutionResult.Failure("No launchable apps were found.")
        val names = apps.joinToString(", ") { it.label }
        return ExecutionResult.Success("${apps.size} apps found: $names")
    }

    /** #10 APP INTELLIGENCE — "close the current app if possible". Android gives a regular,
     * non-system app no public API to force-close another app — that's a deliberate platform
     * restriction. Going home is the closest legitimate alternative (it backgrounds whatever
     * was in the foreground), and the result message says exactly that rather than claiming
     * the app was actually terminated. */
    private fun closeCurrentApp(): ExecutionResult {
        launch(Intent(Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_HOME))
        return ExecutionResult.Success(
            "Android doesn't allow one app to force-close another, so I've returned to the home screen instead — that's the closest JARVIS can legitimately do."
        )
    }
